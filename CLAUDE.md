...
| Python tests | `pytest tests/test_pywvschooldata.py -v`                                       | Python wrapper works correctly                 |
...
system("pip install -e ./pywvschooldata && pytest tests/test_pywvschooldata.py -v")
...
warnings - \[ \] `pytest tests/test_pywvschooldata.py` — all tests
pass - \[ \]
...
**NOT in tidy enrollment:** West Virginia WVDE PDF data provides only
grade-level enrollment, NOT demographic breakdowns (`white`, `black`,
`hispanic`, etc.), gender (`male`, `female`), or special populations
(`special_ed`, `lep`, `econ_disadv`). Demographic data would need to
come from ZoomWV or other WVDE sources.
...
**West Virginia-specific:** `UG` (Ungraded) is a grade level present in
WV data. Raw column names like `grade_k`, `grade_01` etc. are mapped to
standard codes. Always filter on the mapped values.
...
Charter status via `charter_flag` column (`"Y"` = charter). Note: WVDE
PDF data may not include campus-level records.
