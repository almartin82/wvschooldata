# ==============================================================================
# LIVE Pipeline Tests for wvschooldata
# ==============================================================================
#
...
test_that("West Virginia DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add West Virginia DOE URL and verify HTTP 200")
})
...
test_that("Can download West Virginia enrollment data file", {
  skip_if_offline()
...
test_that("Can parse West Virginia enrollment file", {
  skip_if_offline()
...
test_that("wvschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- wvschooldata::get_available_years()
  ...
  tryCatch({
    raw <- wvschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- wvschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- wvschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- wvschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- wvschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- wvschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- wvschooldata:::get_cache_path(2024, "enrollment")
