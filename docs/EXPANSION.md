...
The `wvschooldata` package currently supports enrollment data only (FTE
and headcount from WVDE School Finance PDFs, years 2014-2025).
...
The WV Balanced Scorecard provides publicly accessible CSV files with
graduation rate data at state, district, and school levels with subgroup
breakdowns.
...
**2018 Schema** (48 columns - DIFFERENT): - No `Subgroup_Sort` column
(column 8) - Has `Academic_ELA_Progress_Color_O` and
`Academic_Math_Progress_Color_O` extra columns - Graduation rate fields
are at columns 37 (4-year) and 40 (5-year)
...
- **District Code**: 3-digit zero-padded (002=Barbour, 004=Berkeley, …
  999=State)
